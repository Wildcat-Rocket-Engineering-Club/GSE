...
ADS1256::ADS1256_Result ADS1256::cycleSingle3Tracked(const uint8_t* channels, uint8_t channelCount)
{
    ADS1256_Result result;
    result.channel = 0xFF;
    result.value = 0;
    ...
    updateMUX(channels[currentIndex]);
    ...
    _spi->transfer(0b00000001); // RDATA
    delayMicroseconds(7);
    ...
    _outputBuffer[0] = _spi->transfer(0x0F);
    _outputBuffer[1] = _spi->transfer(0x0F);
    _outputBuffer[2] = _spi->transfer(0x0F);

    _outputValue = ((long)_outputBuffer[0] << 16) |
                   ((long)_outputBuffer[1] << 8) |
                   (_outputBuffer[2]);
    ...
    // Return previous channel (pipeline alignment)
    if (!firstRun) {
        result.channel = channels[previousIndex];
        result.value = _outputValue;
    } else {
        firstRun = false;
    }
