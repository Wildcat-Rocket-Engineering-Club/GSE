...
# This code takes the data given by WREC Static Fire 4
...
fileName1 = r'u:\Documents\Documents\School\Clubs\WREC\Data\WinDaq\StaticFire4\RawData\StaticFire4-TrimmedData.csv'
...
# Set x limits from 0 to 85 seconds
plt.xlim(0, 85)
...
filePath = "U:\Documents\Documents\School\Clubs\WREC\Data\WinDaq\StaticFire4\Graphs\WholeThing"
...
# Plot just the chamber presssure between 75 and 85 seconds.
plt.figure(figsize=(15, 7))
plt.plot(time1, chamber_pressure1, label='Chamber Pressure', color='red')
plt.plot(time1, tank_pressure1, label='Tank Pressure', color='green')
pMax_Plot = 550
...
plt.title('Static Fire 4 Burn')
plt.ylabel('Pressure (psi)')
plt.xlabel('Time (s)')
# Set y axis gridlines to every 25 PSI
plt.yticks(np.arange(0, pMax_Plot + 1, 25))
...
plt.xticks(np.arange(t0, tf + 1, 1))
...
# and update this image name to the next version
fileName = "BurnGraph"
filePath = "U:\Documents\Documents\School\Clubs\WREC\Data\WinDaq\StaticFire4\Graphs\Burn"
output_file = os.path.join(filePath, fileName)
...
print("Static Fire 4 Thrust and Impulse Analysis")
pl()
...
ax1.set_title('Static Fire 4 Thrust')
ax1.set_ylabel('Thrust (N)')
ax1.set_ylim(950, max(thrust_newtons) + 50)
ax1.set_xlabel('Time (s)')
ax1.grid()
ax1.legend(loc='lower left')

# Set major gridlines every 50 Newtons
ax1.set_yticks(np.arange(950, max(thrust_newtons) + 50, 50))
...
filePath = "U:\Documents\Documents\School\Clubs\WREC\Data\WinDaq\StaticFire4\Graphs\ThrustImpulse"
output_file = os.path.join(filePath, fileName)
i = 1
while True:
    fileName = output_file + str(i) + '.png'
...
filePath = "U:\Documents\Documents\School\Clubs\WREC\Data\WinDaq\StaticFire4\ThrustCurve_CSVs"
output_file = os.path.join(filePath, fileName)
